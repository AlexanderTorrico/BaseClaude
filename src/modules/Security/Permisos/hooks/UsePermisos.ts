...
export const usePermisos = () => {
  const data = useSelector((state: RootState) => state.permisos.data);
  const loading = useSelector((state: RootState) => state.permisos.loading);
  const error = useSelector((state: RootState) => state.permisos.error);
  ...
  const fetchData = async (
    ...
    return await PermisosController.getData();
  };
