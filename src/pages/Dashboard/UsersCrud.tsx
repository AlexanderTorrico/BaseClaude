...
// Extended User type for dashboard with additional fields
interface DashboardUser extends Omit<User, 'status'> {
  department: string;
  phone: string;
  joinDate: string;
...
const generateFakeUsers = (): DashboardUser[] => {
...
const UsersCrud: React.FC = () => {
  const [users, setUsers] = useState<DashboardUser[]>(generateFakeUsers());
  ...
  const handleEditUser = (user: DashboardUser): void => {
    setIsEdit(true);
  ...
  const handleDeleteUser = (user: DashboardUser): void => {
    setSelectedUser(user);
    setDeleteModal(true);
  };
