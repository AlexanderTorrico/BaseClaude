...
const Icon = ({ 
  ...
  const renderFeatherIcon = () => {
    try {
      const FeatherIcon = require(`react-feather`)[name];
      if (FeatherIcon) {
        return (
          <FeatherIcon
  ...
  const renderCustomIcon = () => {
    if (React.isValidElement(name)) {
      return React.cloneElement(name, {
        ...
        onKeyDown: onClick ? (e) => e.key === 'Enter' && onClick(e) : name.props.onKeyDown,
        ...props
      });
    }
    return renderFallback();
...
Icon.propTypes = {
  ...
  library: PropTypes.oneOf(['feather', 'bootstrap', 'fontawesome'])
};
