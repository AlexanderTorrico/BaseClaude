...
interface CrudV1User extends Omit<User, 'status'> {
  department: string;
  phone: string;
  joinDate: string;
  salary: number;
...
const generateUsers = (): CrudV1User[] => {
...
const UsersCrudV1: React.FC = () => {
  const [usuarios, setUsuarios] = useState<CrudV1User[]>(generateUsers());
  const [modoVista, setModoVista] = useState<'cards' | 'table'>('cards');
  const [modalAbierto, setModalAbierto] = useState(false);
  const [modalEliminar, setModalEliminar] = useState(false);
  const [modalBusquedaAvanzada, setModalBusquedaAvanzada] = useState(false);
  const [usuarioSeleccionado, setUsuarioSeleccionado] = useState<CrudV1User | null>(null);
  const [usuariosSeleccionados, setUsuariosSeleccionados] = useState<CrudV1User[]>([]);
  const [esEdicion, setEsEdicion] = useState(false);
  const [terminoBusqueda, setTerminoBusqueda] = useState('');
  const [busquedaAvanzada, setBusquedaAvanzada] = useState<Record<string, any>>({});
  const [panelBusquedaAbierto, setPanelBusquedaAbierto] = useState(true);
  const [datosFormulario, setDatosFormulario] = useState<CrudV1FormData>({
    nombre: "",
    email: "",
    telefono: "",
    rol: "Usuario",
  ...
  const manejarAgregarUsuario = (): void => {
    setEsEdicion(false);
    setDatosFormulario({
      nombre: "",
      email: "",
  ...
  const manejarEditarUsuario = (usuario: CrudV1User): void => {
    setEsEdicion(true);
  ...
  const manejarEliminarUsuario = (usuario: CrudV1User): void => {
    setUsuarioSeleccionado(usuario);
    setModalEliminar(true);
  };

  const manejarGuardarUsuario = (): void => {
