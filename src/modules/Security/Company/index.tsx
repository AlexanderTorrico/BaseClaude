...
const Company: React.FC = () => {
  ...
  // Siempre habrá exactamente 1 compañía
  ...
  const handleSubmitCompany = async (dto: any) => {
    if (!currentCompany) return { success: false, message: 'No hay compañía para actualizar' };

    // Actualizar la compañía existente (incluye sucursales actuales)
  ...
  const confirmDeleteBranch = async () => {
    if (!currentCompany || !branchToDelete) return;

    await deleteBranch(currentCompany.id, branchToDelete);
            ...
            <Card className="shadow-sm">
              <CardBody>
                <div className="d-flex align-items-center">
                  ...
                  <div>
                    <h6 className="mb-0">{currentCompany?.name || 'Mi Compañía'}</h6>
                        ...
                        {currentCompany?.sucursales.map(branch => (
                          <Col key={branch.id} xs={12} md={6} className="mb-3">
                            <Card
                              ...
                              onClick={() => {
                                if (branch.lat !== null && branch.lng !== null && branchToDelete !== branch.id) {
                              ...
                              <CardBody className="position-relative">
                                ...
                                {branchToDelete === branch.id && (
                                  <div
                                    ...
                                    style={{ zIndex: 10, borderRadius: '0.25rem' }}
                                  >
                                    <i className="mdi mdi-alert-circle-outline text-danger mb-3" style={{ fontSize: '3rem' }}></i>
                                    <h6 className="mb-2 text-center">¿Eliminar esta sucursal?</h6>
                                    <p className="text-muted text-center mb-4" style={{ fontSize: '0.875rem' }}>
